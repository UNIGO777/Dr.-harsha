export function createLiverHealthOpenAIHandler(getContext) {
  return async (req, res) => {
    try {
      ...
      const liverHealth = await generateLiverHealthWithAi({
        openai,
        provider,
      ...
      res.json({ patient: normalized.patient, liverHealth });
    } catch (err) {
